...
import base64
import logging
from typing import Optional, Dict, Any, List, TypeVar, AsyncIterator, Callable, Awaitable
...
class _BaseWebSocket:
    """Shared WebSocket functionality for all streaming endpoints."""
    ...
    def __aiter__(self) -> "_BaseWebSocket":
        return self
...
class MarketDataWebSocket(_BaseWebSocket):
...
class ExecutorsWebSocket(_BaseWebSocket):
    """WebSocket client for streaming executor data.

    Usage::

        async with client.ws.executors() as ws:
            sub_id = await ws.subscribe_executor_summary()
            async for msg in ws:
                print(msg["type"], msg.get("data"))
    """

    async def subscribe_executors(
        self,
        update_interval: float = 2.0,
        filters: Optional[Dict[str, Any]] = None,
    ) -> str:
        """Subscribe to a filtered executor list.

        Args:
            update_interval: How often the server pushes updates (0.5–60s)
            filters: Optional filter dict with keys: status, controller_id,
                connector_name, trading_pair, executor_type, account_name
    ...
    async def subscribe_executor_detail(
        self,
        executor_id: str,
        update_interval: float = 2.0,
    ) -> str:
        """Subscribe to a single executor's state.

        Args:
            executor_id: The executor ID to monitor
            update_interval: How often the server pushes updates (0.5–60s)
    ...
    async def subscribe_executor_summary(
        self,
        update_interval: float = 2.0,
    ) -> str:
        """Subscribe to aggregate executor summary.
    ...
    async def subscribe_performance(
        ...
        """Subscribe to PnL performance report.
    ...
    async def subscribe_positions(
        ...
        """Subscribe to positions with unrealized PnL.
    ...
    async def subscribe_executor_logs(
        self,
        executor_id: str,
        update_interval: float = 2.0,
        ...
        """Subscribe to streaming logs for an executor.

        Args:
            executor_id: The executor ID to get logs for
            update_interval: How often the server pushes updates (0.5–60s)
            level: Optional log level filter
            limit: Optional max number of log entries
...
class WebSocketRouter:
    ...
    def market_data(self) -> "_WSContext[MarketDataWebSocket]":
        ...
        Usage::
            ...
            async with client.ws.market_data() as ws:
                ...
                async for msg in ws:
                    print(msg)
        """
        return _WSContext(self._connect_ws("/ws/market-data", MarketDataWebSocket))

    def executors(self) -> "_WSContext[ExecutorsWebSocket]":
        """Open a WebSocket connection for streaming executor data.

        Usage::

            async with client.ws.executors() as ws:
                await ws.subscribe_executor_summary()
                async for msg in ws:
                    print(msg)
        """
        return _WSContext(self._connect_ws("/ws/executors", ExecutorsWebSocket))

    async def _connect_ws(self, path: str, cls: type) -> _BaseWebSocket:
        url = self._ws_url(f"{path}?token={self._auth_token()}")
        ws = await self._session.ws_connect(url)
        instance = cls(ws)
        # Wait for the "connected" message
        msg = await instance.receive()
        if msg.get("type") == "error":
            await ws.close()
            raise RuntimeError(f"WebSocket auth failed: {msg.get('message')}")
        instance._connection_id = msg.get("connection_id")
        return instance


_T = TypeVar("_T", bound=_BaseWebSocket)


class _WSContext:
    """Generic async context manager for WebSocket connections."""

    def __init__(self, coro):
        self._coro = coro
        self._ws: Optional[_BaseWebSocket] = None
